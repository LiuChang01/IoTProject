...
#include <string.h>
#include <strings.h>
#define LOG_MODULE "mqtt-sensor-humidity"
...
PROCESS_NAME(mqtt_sensor_humidity);
AUTOSTART_PROCESSES(&mqtt_sensor_humidity);
...
PROCESS(mqtt_sensor_humidity,"MQTT Sensor for humidity");
static bool increasing_humidity=false;
static bool descreasing_humidity=false;
...
static void pub_handler(const char *topic,uint16_t topic_len,const uint8_t *chunk,uint16_t chunk_len){
  ...
  if(strcmp(topic,"waterspurt")==0){
    LOG_INFO("humidity command\n");
    if(strcmp((const char*)chunk,"on")==0){
      LOG_INFO("switch on hum act\n");
      increasing_humidity=true;
      descreasing_humidity=false;
    }else if(strcmp((const char *)chunk,"off")==0){
      LOG_INFO("switch off hum act\n");
      increasing_humidity=false;
      descreasing_humidity=true;
    }
  }else{
...
static void mqtt_event(struct mqtt_connection *m,mqtt_event_t event,void*data){
  switch(event){
    ...
    case MQTT_EVENT_DISCONNECTED:{
      ...
      process_poll(&mqtt_sensor_humidity);
      break;
    }
...
PROCESS_THREAD(mqtt_sensor_humidity,ev,data){
  PROCESS_BEGIN();
  ...
  //broker registrtion
  mqtt_register(&conn,&mqtt_sensor_humidity,sensor_id,mqtt_event,MAX_TCP_SEGMENT_SIZE);
  state=STATE_INIT;
    ...
    if((ev==PROCESS_EVENT_TIMER&&data==&periodic_timer)||ev==PROCESS_EVENT_POLL){
      if(state==STATE_INIT){
        ...
        if(state==STATE_SUBSCRIBED){
            ...
            if(increasing_humidity||descreasing_humidity){
              variation=(rand()%10)+1;//valuein 1-10
              humidity=(increasing_humidity)?(humidity+variation):(humidity-variation);
            }else{
              if((rand()%10)<6){
                variation=(rand()%9)-4;//value in -4 4
                humidity=humidity+variation;
              }
            }
            ...
            LOG_INFO("publish something with humidity->%d\n",humidity);
            sprintf(app_buffer,"{\"node\":%d,\"humidity\":%d}",node_id,humidity);
